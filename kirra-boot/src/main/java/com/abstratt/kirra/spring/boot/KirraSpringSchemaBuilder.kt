...
class KirraSpringSchemaBuilder : SchemaBuilder {
    ...
    fun buildEntity(namespaceName: String, entityType: EntityType<*>): Entity {
        ...
        newEntity.isRole = entityAsKotlinClass.isSubclassOf(IUserProfile::class)
        newEntity.isRole = entityAsKotlinClass.isSubclassOf(IRoleEntity::class)
    ...
    private fun buildStoredProperty(attribute: Attribute<*, *>, tmpObject : Any): Property {
        ...
        property.isUnique = attribute.javaMember.getAnnotations().findAnnotationByType(Column::class)?.unique ?: false
    ...
    inline private fun <R, reified T : Annotation> Attribute<*,*>.getAnnotationValue(property: KProperty1<T, R>): R? =
        this.javaMember.getAnnotations().findAnnotationByType(T::class)?.getValue(property)
    ...
    inner class KotlinJPAProperty(val attribute : Attribute<*,*>, property : KAnnotatedElement) : KotlinDataElement<KAnnotatedElement>(property) {
    ...
    private fun getTypeRef(dataElement: KotlinDataElement<*>): TypeRef {
        ...
        val typeRef = when (typeName) {
            ...
            LocalTime::class.qualifiedName -> TypeRef("kirra" , "Time", TypeRef.TypeKind.Primitive)
                ...
                kirraSpringMetamodel.isEntity(javaType) -> getTypeRef(javaType.java, TypeRef.TypeKind.Entity)
